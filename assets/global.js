...
function resetCardVariantPicker(picker) {
  const toggle = picker.querySelector('[data-card-picker-toggle]');
  const toggleLabel = picker.querySelector('[data-card-picker-label]');
  const panel = picker.querySelector('[data-card-picker-panel]');
  const selectedVariantLabel = picker.querySelector('[data-card-selected-variant]');
  ...
  if (toggleLabel) toggleLabel.textContent = toggle.dataset.defaultLabel || '';
  toggle.removeAttribute('data-selected-variant-id');

  if (selectedVariantLabel) {
    selectedVariantLabel.hidden = true;
    selectedVariantLabel.textContent = '';
  }
}

function closeCardVariantPickers(exceptPicker = null) {
  document.querySelectorAll('[data-card-variant-picker]').forEach((picker) => {
...
function addCardVariantToCart({ variantId, button, select }) {
  if (!variantId) return;
  ...
  fetch(`${theme.routes.cart_add_url}`, { ...fetchConfig('javascript'), body })
    ...
    .then((parsedState) => {
      ...
      miniCart && miniCart.renderContents(parsedState);
...
function getUppercaseAddToCartLabel(fallback = 'ADD TO CART') {
  const label = theme?.variantStrings?.addToCart || fallback;
  return String(label).toUpperCase();
}

function normalizeCardSizeValue(value) {
  return String(value || '')
    .trim();
}

function isMeaningfulCardSizeValue(value) {
  const normalized = normalizeCardSizeValue(value)
    .replace(/\s+/g, '')
    .replace(/[.,\-/+]/g, '');
  ...
  if (['0', '0.0', '0.00'].includes(normalized)) {
    return false;
  }

  const alphaOnly = normalized.replace(/[0-9]/g, '');
  return alphaOnly.length > 0;
}

function splitCardSizeValue(value) {
  const rawValue = normalizeCardSizeValue(value);
  if (!isMeaningfulCardSizeValue(rawValue)) {
  ...
  return {
    main: normalized[0],
    secondary: normalized.slice(1).join(' | '),
  };
}

function updateSizeDisplay(picker, value) {
  if (!picker) return;

  const wrapper = picker.closest('.card-wrapper, .bestseller-card');
  const display = wrapper?.querySelector('[data-size-display-value]');
  if (!wrapper || !display) return;

  const main = wrapper.querySelector('[data-size-display-main]');
  const parts = splitCardSizeValue(value || display.textContent);
  const shouldShow = Boolean(parts.main);
  ...
  const secondary = wrapper.querySelector('[data-size-display-secondary]');
  const separator = wrapper.querySelector('[data-size-display-separator]');
  if (secondary) secondary.hidden = !parts.secondary;
  if (separator) separator.hidden = !parts.secondary;
}

function updateCardVariantPrice(picker, variantOption) {
  if (!picker || !variantOption) return;

  const wrapper = picker.closest('.card-wrapper, .bestseller-card');
  if (!wrapper) return;

  const priceText = variantOption.dataset.variantPrice || '';
  const compareText = variantOption.dataset.variantCompare || '';

  const currentPriceTargets = wrapper.matches('.bestseller-card')
    ? ['.bestseller-card__price']
    : [
        '.price__sale .price-item--sale price-money bdi',
        '.price__sale .price-item--sale bdi',
        '.price__regular .price-item--regular price-money bdi',
        '.price__regular .price-item--regular bdi',
      ];

  const comparePriceTargets = wrapper.matches('.bestseller-card')
    ? ['.bestseller-card__compare']
    : [
        '.price__compare .price-item--regular price-money bdi',
        '.price__compare .price-item--regular bdi',
        '.price__sale .price__compare s.price-item--regular price-money bdi',
        '.price__sale .price__compare s.price-item--regular bdi',
      ];

  currentPriceTargets.forEach((selector) => {
    wrapper.querySelectorAll(selector).forEach((target) => {
      target.textContent = priceText;
    });
  });

  comparePriceTargets.forEach((selector) => {
    wrapper.querySelectorAll(selector).forEach((target) => {
      target.textContent = compareText;
      const compareWrapper = target.closest('.price__compare, s, dd, .bestseller-card__compare');
      if (compareWrapper) {
        compareWrapper.hidden = !compareText;
      }
    });
  });
}

function getVariantOptionElements(picker) {
  if (!picker) return [];

  return Array.from(picker.querySelectorAll('[data-card-variant-option], [data-bestseller-variant-option]'));
}

function updateVariantOptionSelectionState(picker, selectedOption) {
  if (!picker) return;

  getVariantOptionElements(picker).forEach((option) => {
...
function initializeVariantPickerDefaults(root = document) {
  if (!root) return;

  root.querySelectorAll('[data-card-variant-picker], [data-bestseller-variant-picker]').forEach((picker) => {
    if (picker.dataset.variantPickerDefaultsInit === '1') return;
    picker.dataset.variantPickerDefaultsInit = '1';

    const options = getVariantOptionElements(picker);
    ...
    updateVariantOptionSelectionState(picker, selectedOption);
    updateSizeDisplay(picker, selectedOption.dataset.variantOption || selectedOption.textContent || '');
    updateCardVariantPrice(picker, selectedOption);
  });
}

function focusFirstEnabledVariantOption(picker) {
  const firstOption = getVariantOptionElements(picker).find((option) => !option.disabled);
  if (firstOption) firstOption.focus();
}

function focusSelectedVariantOption(picker) {
  const selectedOption = getVariantOptionElements(picker).find((option) => option.getAttribute('aria-selected') === 'true');
  if (selectedOption) {
    selectedOption.focus();
    return;
  }

  focusFirstEnabledVariantOption(picker);
}

function focusSiblingVariantOption(currentOption, step) {
  if (!currentOption || !step) return;

  const picker = currentOption.closest('[data-card-variant-picker], [data-bestseller-variant-picker]');
  const options = getVariantOptionElements(picker).filter((option) => !option.disabled);
...
function toggleBestsellerVariantPicker(picker) {
  const panel = picker.querySelector('[data-bestseller-picker-panel]');
  const toggle = picker.querySelector('[data-bestseller-picker-toggle]');
  if (!panel || !toggle) return;

  const isOpen = picker.classList.contains('is-open');
  document.querySelectorAll('[data-bestseller-variant-picker].is-open').forEach((openPicker) => {
    if (openPicker !== picker) {
      openPicker.classList.remove('is-open');
      const openPanel = openPicker.querySelector('[data-bestseller-picker-panel]');
      const openToggle = openPicker.querySelector('[data-bestseller-picker-toggle]');
  ...
  focusSelectedVariantOption(picker);
}

function resetBestsellerVariantPicker(picker) {
  const toggle = picker.querySelector('[data-bestseller-picker-toggle]');
  const panel = picker.querySelector('[data-bestseller-picker-panel]');
  const selectedVariantLabel = picker.querySelector('[data-bestseller-selected-variant]');
  if (!toggle || !panel) return;
  ...
  toggle.setAttribute('aria-expanded', 'false');
  toggle.removeAttribute('data-selected-variant-id');
  toggle.textContent = toggle.dataset.defaultLabel || '';
  if (selectedVariantLabel) {
    selectedVariantLabel.hidden = true;
    selectedVariantLabel.textContent = '';
  }
...
function initBestsellerCardSliders(root = document) {
  if (!root) return;

  root.querySelectorAll('[data-bestseller-card]').forEach((card) => {
    if (card.dataset.bestsellerCardInit === '1') return;
    card.dataset.bestsellerCardInit = '1';
    ...
    const setSlide = (n) => {
      ...
      slides.forEach((slide, slideIndex) => slide.classList.toggle('is-active', slideIndex === idx));
      if (dotsWrap) {
        dotsWrap.querySelectorAll('button').forEach((dot, dotIndex) => dot.classList.toggle('is-active', dotIndex === idx));
      }
    };
    ...
    if (dotsWrap) {
      ...
      dotsWrap.innerHTML = slides
        .map((_, slideIndex) => `<button type="button" class="${slideIndex === 0 ? 'is-active' : ''}" aria-label="Slide ${slideIndex + 1}"></button>`)
        .join('');
...
function closeBestsellerVariantPickers(exceptPicker = null) {
  document.querySelectorAll('[data-bestseller-variant-picker]').forEach((picker) => {
    if (exceptPicker && picker === exceptPicker) return;
...
document.addEventListener('click', (event) => {
  const toggle = event.target.closest('[data-card-picker-toggle]');
  if (toggle) {
    const picker = toggle.closest('[data-card-variant-picker]');
    const panel = picker && picker.querySelector('[data-card-picker-panel]');
    if (!picker || !panel) return;

    const selectedVariantId = toggle.dataset.selectedVariantId;
    if (selectedVariantId) {
      addCardVariantToCart({
        variantId: selectedVariantId,
        button: toggle,
      });
      resetCardVariantPicker(picker);
      return;
    }

    const isOpen = picker.classList.contains('is-open');
    closeCardVariantPickers(isOpen ? null : picker);

    if (isOpen) {
      closeCardVariantPickers();
      return;
    }
  ...
  if (variantOption) {
    const picker = variantOption.closest('[data-card-variant-picker]');
    const toggle = picker && picker.querySelector('[data-card-picker-toggle]');
    const toggleLabel = picker && picker.querySelector('[data-card-picker-label]');
    const panel = picker && picker.querySelector('[data-card-picker-panel]');
    const selectedVariantLabel = picker && picker.querySelector('[data-card-selected-variant]');
    const variantsJson = picker && picker.querySelector('[data-card-variants]');
    const variants = variantsJson ? JSON.parse(variantsJson.textContent) : [];
    if (!picker || !toggle || !panel) return;

    const selectedVariantId = variantOption.dataset.variantId;
    ...
    toggle.dataset.selectedVariantId = selectedVariant.id;
    if (toggleLabel) toggleLabel.textContent = picker.dataset.addLabel || getUppercaseAddToCartLabel();
    picker.classList.remove('is-open');
    panel.hidden = true;
    toggle.setAttribute('aria-expanded', 'false');

    if (selectedVariantLabel) {
      selectedVariantLabel.hidden = false;
      selectedVariantLabel.textContent = `${picker.dataset.optionName}: ${selectedVariant.option}`;
    }
    updateVariantOptionSelectionState(picker, variantOption);
    updateSizeDisplay(picker, selectedVariant.option);
    updateCardVariantPrice(picker, variantOption);
    return;
  }

  if (!event.target.closest('[data-card-variant-picker]')) {
    closeCardVariantPickers();
  }

  if (!event.target.closest('[data-bestseller-variant-picker]')) {
    closeBestsellerVariantPickers();
  }

  const bestsellerToggle = event.target.closest('[data-bestseller-picker-toggle]');
  if (bestsellerToggle) {
    const picker = bestsellerToggle.closest('[data-bestseller-variant-picker]');
    if (!picker) return;

    const selectedVariantId = bestsellerToggle.dataset.selectedVariantId;
    if (selectedVariantId) {
      addCardVariantToCart({
        variantId: selectedVariantId,
        button: bestsellerToggle,
      });
      resetBestsellerVariantPicker(picker);
      return;
    }

    toggleBestsellerVariantPicker(picker);
    return;
  }

  const bestsellerOption = event.target.closest('[data-bestseller-variant-option]');
  if (bestsellerOption) {
    const picker = bestsellerOption.closest('[data-bestseller-variant-picker]');
    const toggle = picker && picker.querySelector('[data-bestseller-picker-toggle]');
    const panel = picker && picker.querySelector('[data-bestseller-picker-panel]');
    const selectedVariantLabel = picker && picker.querySelector('[data-bestseller-selected-variant]');
    const variantsJson = picker && picker.querySelector('[data-bestseller-variants]');
    const variants = variantsJson ? JSON.parse(variantsJson.textContent) : [];
    if (!picker || !toggle || !panel) return;

    const selectedVariantId = bestsellerOption.dataset.variantId;
    const selectedVariant = variants.find((variant) => String(variant.id) === String(selectedVariantId));
    if (!selectedVariant || !selectedVariant.available) return;

    toggle.dataset.selectedVariantId = selectedVariant.id;
    toggle.textContent = picker.dataset.addLabel || getUppercaseAddToCartLabel();
    picker.classList.remove('is-open');
    panel.hidden = true;
    toggle.setAttribute('aria-expanded', 'false');
    ...
    updateVariantOptionSelectionState(picker, bestsellerOption);
    updateSizeDisplay(picker, selectedVariant.option);
    updateCardVariantPrice(picker, bestsellerOption);
    return;
  }
});

document.addEventListener('keydown', (event) => {
  const option = event.target.closest('[data-card-variant-option], [data-bestseller-variant-option]');
  if (!option) return;

  if (event.key === 'ArrowDown' || event.key === 'ArrowRight') {
    event.preventDefault();
    focusSiblingVariantOption(option, 1);
    return;
  }
  ...
  if (event.key === 'Home') {
    event.preventDefault();
    const picker = option.closest('[data-card-variant-picker], [data-bestseller-variant-picker]');
    focusFirstEnabledVariantOption(picker);
    return;
  }
  ...
  if (event.key === 'End') {
    event.preventDefault();
    const picker = option.closest('[data-card-variant-picker], [data-bestseller-variant-picker]');
    const enabledOptions = getVariantOptionElements(picker).filter((item) => !item.disabled);
  ...
  if (event.key === 'Escape') {
    ...
    const panel = picker.querySelector('[data-card-picker-panel], [data-bestseller-picker-panel]');
    const toggle = picker.querySelector('[data-card-picker-toggle], [data-bestseller-picker-toggle]');
    picker.classList.remove('is-open');
  ...
  handleIntersection(entries, observer) {
    ...
    fetch(this.dataset.url)
      ...
      .then(text => {
        ...
        if (recommendations && recommendations.innerHTML.trim().length) {
          this.innerHTML = recommendations.innerHTML;
          initBestsellerCardSliders(this);
          initializeVariantPickerDefaults(this);
        }
      })
      .catch(e => {
        console.error(e);
...
document.addEventListener('page:loaded', () => {
  initializeVariantPickerDefaults(document);
});
